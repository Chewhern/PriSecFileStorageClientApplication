    ...
    public partial class UserGenerateKey : Form
    {
        ...
        private void ConfirmAccessBTN_Click(object sender, EventArgs e)
        {
                ...
                && OwnerStorageIDTB.Text!=null && OwnerStorageIDTB.Text.CompareTo("")!=0) 
            {
                ...
                if(Directory.Exists(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + OwnerStorageID) == false) 
                {
                    Directory.CreateDirectory(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + OwnerStorageID);
                }
                File.WriteAllText(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + OwnerStorageID+"\\AccessID.txt",AccessID);
                File.WriteAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + OwnerStorageID + "\\SK.txt",ED25519SK);
                File.WriteAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + OwnerStorageID + "\\PK.txt", ED25519PK);
                File.WriteAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + OwnerStorageID + "\\SPK.txt", SignedED25519PK);
            ...
            {
                MessageBox.Show("Please Generate Signature KeyPair and get access ID + storage ID from owner","Information",MessageBoxButtons.OK,MessageBoxIcon.Error);
            }
        }
