...
namespace PriSecFileStorageClient
{
    public partial class OwnerUploadFile : Form
    {
        ...
        private async void UploadServerBTN_Click(object sender, EventArgs e)
        {
            if (RandomFileNameTB.Text.CompareTo("") != 0)
            {
                String RandomFileName = RandomFileNameTB.Text;
                int FilesCount = Directory.GetFiles(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName).Length;
                ...
                await Task.Run(() => BackGroundUpload(progress));
                String FileStorageUsedSizeString = "";
                GetFileStorageUsedSize(ref FileStorageUsedSizeString);
                FileStorageSizeTB.Text = FileStorageUsedSizeString;
            }
            else
            {
                MessageBox.Show("You have not yet encrypt file", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
                ...
                if (FileSize<= 1073741824) 
                {
                    if(Directory.Exists(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryID) == true) 
                    {
                        ...
                        else
                        {
                            ...
                            await Task.Run(() => BackGroundEncrypt(progress));
                        }
                    }
                    else 
                    {
        ...
        public void BackGroundUpload(IProgress<int> progress)
        {
        ...
        public void BackGroundEncrypt(IProgress<int> progress) 
        {
            int FileSize = int.Parse(FileSizeTB.Text);
            String DirectoryID = DirectoryIDTempStorage.DirectoryID;
            String FileName = PlainTextFileChooserDialog.SafeFileName;
