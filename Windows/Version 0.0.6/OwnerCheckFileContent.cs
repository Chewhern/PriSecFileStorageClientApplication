...
namespace PriSecFileStorageClient
{
    public partial class OwnerCheckFileContent : Form
    {
        ...
        private async void CheckFileContentsBTN_Click(object sender, EventArgs e)
        {
            if(DirectoryIDTempStorage.DirectoryID.CompareTo("")!=0 && EncryptedRandomFileNameCB.Text.CompareTo("")!=0 && FileContentCountTB.Text.CompareTo("") != 0) 
            {
                ...
                {
                    CheckFileContentStatusLB.Items.Add(percent);
                });

                await Task.Run(() => BackGroundCompareFileContent(progress,RandomFileName));
        ...
        public Byte[] GetLocalFileContent(String DirectoryID,String RandomFileName,int Count) 
        {
            try 
            {
                Byte[] ResultByte=File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\FileContent" + Count.ToString() + ".txt");
                return ResultByte;
            }
            catch 
            {
        ...
        public void BackGroundCompareFileContent(IProgress<String> progress, String RandomFileName) 
        {
            int LoopCount = 1;
