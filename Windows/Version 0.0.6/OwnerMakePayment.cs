                            ...
                            if (result.IsSuccessStatusCode)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (DirectoryHolder.Status != null && DirectoryHolder.Status.CompareTo("") != 0)
                                    {
                                        ...
                                        Directory.CreateDirectory(Application.StartupPath + "\\Application_Data\\" + "Server_Directory_Data\\" + DirectoryHolder.FolderID);
                                        File.WriteAllBytes(Application.StartupPath + "\\Application_Data\\" + "Server_Directory_Data\\" + DirectoryHolder.FolderID + "\\rootSK.txt", MyKeyPair.PrivateKey);
                                        File.WriteAllBytes(Application.StartupPath + "\\Application_Data\\" + "Server_Directory_Data\\" + DirectoryHolder.FolderID + "\\rootPK.txt", MyKeyPair.PublicKey);
                                        MyKeyPair.Clear();
                                        return true;
                                    }
                                    else
