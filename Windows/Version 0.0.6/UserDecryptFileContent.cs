    ...
    public partial class UserDecryptFileContent : Form
    {
        ...
        private void EncryptedRandomFileNameCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (EncryptedRandomFileNameCB.SelectedIndex != -1)
            {
                ...
                try
                {
                    OriginalFileNameByte = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + EncryptedRandomFileNameCB.Text + "\\FileName.txt");
                }
                catch
                {
        ...
        {
            if (Directory.Exists(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files") ==true) 
            {
                String[] RandomFileIDFullPathArray = Directory.GetDirectories(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files");
                String[] RandomFileIDArray = new string[RandomFileIDFullPathArray.Length];
                int Count = 0;
                int RootDirectoryCount = 0;
                RootDirectoryCount = (Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\").Length;
        ...
        private async void DecryptFetchedFileContentsBTN_Click(object sender, EventArgs e)
        {
            if (FileContentCountTB.Text.CompareTo("") != 0 && EncryptedRandomFileNameCB.Text.CompareTo("") != 0)
            {
                int FileContentCount = int.Parse(FileContentCountTB.Text);
                DecryptFolderSelector.ShowDialog();
                if (DecryptFolderSelector.SelectedPath.CompareTo("") != 0)
                {
                    String DecryptionFolder = DecryptFolderSelector.SelectedPath;
                    String RandomFileName = EncryptedRandomFileNameCB.Text;
                    Boolean IsXSalsa20Poly1305 = DefaultRB.Checked;
                    Boolean IsXChaCha20Poly1305 = XChaCha20Poly1305RB.Checked;
                    Boolean IsAES256HAGCM = AES256GCMRB.Checked;
                    DecryptProgressBar.Maximum = FileContentCount;
                    ...
                    await Task.Run(() => BackGroundDecryptFileContent(progress, DecryptionFolder, FileContentCount, RandomFileName, IsXSalsa20Poly1305, IsXChaCha20Poly1305, IsAES256HAGCM));
                }
                else
                {
                    MessageBox.Show("You must choose a folder to store the decrypted data", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
                MessageBox.Show("Please check the amount of file parts need to be fetched and decrypted" + Environment.NewLine +
                    "Please choose a server random file name", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void GetFileContentCountBTN_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Have you enter server directory ID?", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public int GetFileContentCount(String DirectoryID, String RandomFileName, Byte[] RandomChallenge)
        {
            ...
            if (ETLSSessionIDStorage.ETLSID.CompareTo("") != 0 && ETLSSessionIDStorage.ETLSID != null)
            {
                ...
                UserECDSASK = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\" + "Other_Directory_Data\\" + DirectoryID + "\\SK.txt");
                AccessID = File.ReadAllText(Application.StartupPath + "\\Application_Data\\" + "Other_Directory_Data\\" + DirectoryID + "\\AccessID.txt");
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var response = client.GetAsync("UserFileOperation/GetEndpointEncryptedFileContentCount?ClientPathID=" + ETLSSessionIDStorage.ETLSID + "&CipheredSignedDirectoryID=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedCombinedCipheredDirectoryIDByte)) + "&SignedSignedRandomChallenge=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedUserSignedRandomChallenge)) + "&SignedUniqueFileName=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedRandomFileNameByte)) + "&CipheredSignedAnotherUserID=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedCombinedCipheredAccessIDByte)));
                    try
                    {
                        response.Wait();
                    }
        ...
        public Byte[] GetFileContent(String DirectoryID, String RandomFileName, Byte[] RandomChallenge, int Count)
        {
            ...
            if (ETLSSessionIDStorage.ETLSID.CompareTo("") != 0 && ETLSSessionIDStorage.ETLSID != null)
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Add(
                        new MediaTypeWithQualityHeaderValue("application/json"));
                    var response = client.GetAsync("UserFileOperation/GetEndpointEncryptedFile?ClientPathID=" + ETLSSessionIDStorage.ETLSID + "&SignedUserID=" + "&CipheredSignedDirectoryID=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedCombinedCipheredDirectoryIDByte)) + "&SignedSignedRandomChallenge=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedUserSignedRandomChallenge)) + "&SignedUniqueFileName=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedRandomFileNameByte)) + "&FileContentCount=" + Count.ToString() + "&CipheredSignedAnotherUserID=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(ETLSSignedCombinedCipheredAccessIDByte)));
                    try
                    {
                        response.Wait();
                    }
        ...
        public void BackGroundDecryptFileContent(IProgress<int> progress, String DecryptionFolderPath, int FileCount, String RandomFileName, Boolean IsXSalsa20Poly1305, Boolean IsXChaCha20Poly1305, Boolean IsAES256HAGCM)
        {
            String DecryptFolderPath = DecryptionFolderPath;
            int LoopCount = 1;
            int Count = FileCount;
            String Base64Challenge = "";
            Byte[] Base64ChallengeByte = new Byte[] { };
            ...
            try
            {
                OriginalFileNameByte = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\FileName.txt");
            }
            catch
            {
                CheckFileNameExists = false;
                ...
                while (LoopCount <= Count)
                {
                    ...
                    if (Base64Challenge != null && Base64Challenge.CompareTo("") != 0)
                    {
                        ...
                        if (ServerFileContent.Length != 0)
                        {
                            try
                            {
                                Key = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\Key\\" + "Key" + LoopCount.ToString() + ".txt");
                            }
                            catch
                            {
                            ...
                            try
                            {
                                ED25519PK = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Other_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\ED25519PK\\" + "PK" + LoopCount.ToString() + ".txt");
                            }
                            catch
                            {
                            ...
                            if (VerifiedServerFileContent.Length == FileBytesChunkClass.NonceMaximumChunksFileBytesWithMACLength)
                            {
