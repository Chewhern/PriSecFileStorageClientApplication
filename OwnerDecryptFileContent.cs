...
namespace PriSecFileStorageClient
{
    public partial class OwnerDecryptFileContent : Form
    {
        ...
        {
            if (Directory.Exists(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files") == true) 
            {
        ...
        public void BackGroundDecryptFileContent(IProgress<int> progress,String DecryptionFolderPath,int FileCount,String RandomFileName,Boolean IsXSalsa20Poly1305,Boolean IsXChaCha20Poly1305, Boolean IsAES256HAGCM)
        {
            ...
            try
            {
                OriginalFileNameByte = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\FileName.txt");
            }
            catch
            {
                CheckFileNameExists = false;
                ...
                while (LoopCount <= Count)
                {
                    ...
                    if (Base64Challenge != null && Base64Challenge.CompareTo("") != 0)
                    {
                        ...
                        if (ServerFileContent.Length != 0)
                        {
                            try
                            {
                                Key = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\Key\\" + "Key" + LoopCount.ToString() + ".txt");
                            }
                            catch
                            {
                            ...
                            try
                            {
                                ED25519PK = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\Server_Directory_Data\\" + DirectoryIDTempStorage.DirectoryID + "\\Encrypted_Files\\" + RandomFileName + "\\ED25519PK\\" + "PK" + LoopCount.ToString() + ".txt");
                            }
                            catch
                            {
